...
public final class TemplateGenerator: TemplateGenerating {
    ...
    private func renderItems(
        ...
        return try template.items.map {
            ...
            if case let Template.Contents.file(path) = contents {
                ...
                contents = .file(
                    try AbsolutePath(validatingAbsolutePath: renderedPathString)
                )
            }
            ...
            return Template.Item(path: path.pathString, contents: contents)
        }
    }
    ...
    private func generateDirectories(
        ...
        try renderedItems
            .map(\.path)
            .map {
                let relPath = try RelativePath(validatingRelativePath: $0)
                let dirname = try RelativePath(validatingRelativePath: relPath.dirname)
                return destinationPath.appending(dirname)
            }
            .forEach {
        ...
        for renderedItem in renderedItems {
            ...
            case let .directory(path):
                let destinationDirectoryPath = destinationPath
                    .appending(try RelativePath(validatingRelativePath: renderedItem.path))
            ...
            if let rendered = renderedContents,
               try !rendered.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ||
               RelativePath(validatingRelativePath: renderedItem.path).basename == ".gitkeep"
            {
                try FileHandler.shared.write(
                    rendered,
                    path: destinationPath.appending(try RelativePath(validatingRelativePath: renderedItem.path)),
                    atomically: true
                )
            }
        }
