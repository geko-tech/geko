...
/// Geko includes all methods to interact with your geko project
public enum Geko {
    enum GekoError: Error {
    ...
    public static func graph(at path: String? = nil, environmentKeys: Set<String> = []) throws -> Graph {
        // If a task is executed via `geko`, it gets passed the binary path as a last argument.
        // Otherwise, fallback to go
        let gekoBinaryPath = ProcessInfo.processInfo.environment["GEKO_CONFIG_BINARY_PATH"] ?? "geko"
        ...
        do {
            ...
            let forceConfigCacheDirectory = "GEKO_CONFIG_FORCE_CONFIG_CACHE_DIRECTORY"
    ...
    private static func run(
        ...
        if process.terminationStatus != 0 {
            ...
            throw Geko.GekoError.terminated(
