...
enum ProjectFileElementsError: FatalError {
    ...
    case buildableFolderElementConflict(AbsolutePath)
    case fileBuildableFolderConflict(AbsolutePath)
    case buildableFolderSameNameConflist(AbsolutePath)

    var description: String {
        switch self {
        case let .buildableFolderConflict(folder, conflictedFolder):
            return "Cannot add \(folder) to project because it conflicts with another buildable folder \(conflictedFolder)"
        case let .buildableFolderElementConflict(folder):
            return "Cannot add \(folder) to project because it conflicts with another file reference or group with the same path"
        case let .fileBuildableFolderConflict(projectPath):
            return
                "Cannot generate project \(projectPath) because it contains files that are already in buildable folders. You can remove them from sources, resources, additionalFiles or other fields."
        case let .buildableFolderSameNameConflist(folderPath):
            return "Buildable folder \(folderPath) cannot be added because it conflicts with group of file with the same name."
        }
    }
...
class ProjectFileElements {
    ...
    private var enforceExplicitDependencies: Bool

    var elements: [AbsolutePath: PBXFileElement] = [:]
    var products: [String: PBXFileReference] = [:]
    var compiled: [AbsolutePath: PBXFileReference] = [:]
    var buildableFolders: [AbsolutePath: PBXFileSystemSynchronizedRootGroup] = [:]
    var knownRegions: Set<String> = Set([])

    // MARK: - Init

    init(enforceExplicitDependencies: Bool, _ elements: [AbsolutePath: PBXFileElement] = [:]) {
    ...
    func generateProjectFiles(
        ...
        let allBuildableFolders = project.targets.reduce(into: []) { acc, target in
            acc.append(
                contentsOf: target.buildableFolders.map {
                    GroupFileElement(
                        path: $0.path,
        ...
        let buildableFolderPaths = allBuildableFolders.map(\.path)
        var files = Set<GroupFileElement>()

        for target in project.targets {
            try files.formUnion(targetFiles(target: target, buildableFolders: buildableFolderPaths))
        }
        let projectFileElements = projectFiles(project: project, buildableFolders: buildableFolderPaths)
        ...
        var buildableFolderConflict = false
        for file in sortedFiles {
            guard let conflictedFolder = allBuildableFolders.first(where: { $0.path.isAncestor(of: file.path) }) else {
                continue
            }

            buildableFolderConflict = true
            logger.error(" · Cannot add file element \(file.path) to project because it conflicts with buildable folder \(conflictedFolder.path)")
        }
        if buildableFolderConflict {
            throw ProjectFileElementsError.fileBuildableFolderConflict(project.path)
        }
        ...
        try generateBuildableFolders(
            folders: allBuildableFolders,
            groups: groups,
            pbxproj: pbxproj,
            sourceRootPath: project.sourceRootPath
        )
    ...
    func projectFiles(project: Project, buildableFolders: [AbsolutePath]) -> Set<GroupFileElement> {
        ...
        fileElements.formUnion(
            configFiles.filter { file in !buildableFolders.contains(where: { $0.isAncestor(of: file) }) }
                .map { GroupFileElement(path: $0, group: project.filesGroup) }
        )

        // Additional files
        fileElements.formUnion(
            project.additionalFiles
                .filter { file in !buildableFolders.contains(where: { $0.isAncestor(of: file.path) }) }
                .map {
                    GroupFileElement(
                        path: $0.path,
                        group: project.filesGroup,
    ...
    func targetFiles(target: Target, buildableFolders: [AbsolutePath]) throws -> Set<GroupFileElement> {
        var files = Set<AbsolutePath>()
        for sourceFiles in target.sources {
            files.formUnion(sourceFiles.paths)
        }
        ...
        if let headers = target.headers {
            headers.list.forEach {
                files.formUnion($0.public?.files ?? [])
                files.formUnion($0.private?.files ?? [])
                files.formUnion($0.project?.files ?? [])
            }
        }

        // Support files
        if let infoPlist = target.infoPlist,
            let path = infoPlist.path,
            !buildableFolders.contains(where: { $0.isAncestor(of: path) })
        {
            files.insert(path)
        }

        if let entitlements = target.entitlements,
            let path = entitlements.path,
            !buildableFolders.contains(where: { $0.isAncestor(of: path) })
        {
            files.insert(path)
        }
        ...
        target.settings?.configurations.xcconfigs().forEach { configFilePath in
            if !buildableFolders.contains(where: { $0.isAncestor(of: configFilePath) }) {
        ...
        // Additional files
        files.formUnion(
            target.additionalFiles.map(\.path)
                .filter { file in !buildableFolders.contains(where: { $0.isAncestor(of: file) }) }
        )
    ...
    func generateBuildableFolders(
        folders: [GroupFileElement],
        groups: ProjectGroups,
        pbxproj: PBXProj,
        sourceRootPath: AbsolutePath
    ) throws {
        let folderSet = Set(folders.map(\.path))

        outer: for folder in folders {
            var foldersToCheck: [AbsolutePath] = []
            var currentFolder = folder.path.parentDirectory
            while !currentFolder.isRoot {
                foldersToCheck.append(currentFolder)
                currentFolder = currentFolder.parentDirectory
            }
            for folderToCheck in foldersToCheck.reversed() {
                if folderSet.contains(folderToCheck) {

                    logger.error("Cannot add \(folder.path) to project because it conflicts with another buildable folder \(folderToCheck)")
                    throw ProjectFileElementsError.buildableFolderConflict(folder.path, folderToCheck)
                }
            }
            if elements[folder.path] != nil {
                throw ProjectFileElementsError.buildableFolderElementConflict(folder.path)
            }

            try generateBuildableFolder(
                fileElement: folder,
                groups: groups,
                pbxproj: pbxproj,
                sourceRootPath: sourceRootPath
    ...
    func generatePrecompiledDependency(
        ...
        if path.pathString.contains(charArray: CacheCategory.builds.directoryName.utf8CString) {
    ...
    private func generateProduct(
        ...
        let fileReference = PBXFileReference(
            sourceTree: enforceExplicitDependencies ? .custom("BASE_CONFIGURATION_BUILD_DIR") : .buildProductsDir,
            name: enforceExplicitDependencies ? productName : nil,
            explicitFileType: fileType,
            path: enforceExplicitDependencies ? "\(targetName)/\(productName)" : productName,
    ...
    func generate(
        ...
        case let .groupReference(name: groupName, path: groupPath):
            group = try groups.projectGroup(named: groupName)
            sourcePath = groupPath
        }
        
        let fileElementRelativeToSourceRoot = fileElement.path.relative(to: sourcePath)
        let closestRelativeRelativePath = try closestRelativeElementPath(pathRelativeToSourceRoot: fileElementRelativeToSourceRoot)
        let closestRelativeAbsolutePath = sourcePath.appending(closestRelativeRelativePath)
    ...
    func generateBuildableFolder(
        ...
        if elements[fileElement.path] != nil {
            throw ProjectFileElementsError.buildableFolderSameNameConflist(fileElement.path)
        }
        ...
        // If it matches the file that we are adding we can exit
        if (closestRelativeAbsolutePath == fileElement.path) || !(firstElement.element is PBXGroup) {
            return
        }
    ...
    @discardableResult func addElement(
        ...
        if elements[absolutePath] != nil {
            return (element: elements[absolutePath]!, path: absolutePath)
        }
        if isLeaf, isBuildableFolder, buildableFolders[absolutePath] != nil {
            // buildable folder is already added, exit early
            return nil
        }
        ...
        } else if isBuildableFolder {
            addBuildableFolderRelativeToGroup(
    ...
    func addBuildableFolderRelativeToGroup(
        fileAbsolutePath: AbsolutePath,
        fileRelativePath: RelativePath,
        name: String?,
        toGroup: PBXGroup,
        ...
        let folder = PBXFileSystemSynchronizedRootGroup(
            sourceTree: .group,
            path: fileRelativePath.pathString,
            name: name
        )
        pbxproj.add(object: folder)
        toGroup.children.append(folder)
        buildableFolders[fileAbsolutePath] = folder
    }
    ...
    private func addSDKElement(
        ...
        // swiftlint:disable:next force_try
        let sdkPath = sdkNodePath.relative(to: try! AbsolutePath(validatingAbsolutePath: "/"))  // SDK paths are relative
    ...
    func buildableFolder(path: AbsolutePath) -> PBXFileSystemSynchronizedRootGroup? {
        buildableFolders[path]
    }
    ...
    func normalize(_ path: AbsolutePath) -> AbsolutePath {
        ...
        if let localizedMatch = ProjectFileElements.localizedRegex.firstMatch(
            ...
            return try! AbsolutePath(validatingAbsolutePath: lprojPath)  // swiftlint:disable:this force_try
        } else {
            return path
        }
    }
