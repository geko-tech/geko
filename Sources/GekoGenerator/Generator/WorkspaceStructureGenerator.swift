...
private class DirectoryStructure {
    ...
    private func isDependencyProject(_ node: Node) -> Bool {
        ...
        case let .project(path):
            return path.pathString
                .contains(
                    "\(Constants.DependenciesDirectory.packageBuildDirectoryName)/\(Constants.DerivedDirectory.dependenciesDerivedDirectory)"
                )
    ...
    private func fileNode(from element: FileElement) -> Node {
        switch element {
        case let .glob(path):
            fatalError("Globs must be expanded before project generation: \(path)")
