...
final class SchemeDescriptorsGenerator: SchemeDescriptorsGenerating {
    ...
    func schemeTestAction(
        ...
        for testableTarget in testAction.targets {
            ...
            let testable = XCScheme.TestableReference(
                ...
                parallelization: testableTarget.isParallelizable ? .all : .none,
    ...
    func schemeLaunchAction(
        ...
        let enableUBSanitizer = scheme.runAction?.diagnosticsOptions.undefinedBehaviorSanitizerEnabled ?? false
        let disableMainThreadChecker = scheme.runAction?.diagnosticsOptions.mainThreadCheckerEnabled == false
        let disablePerformanceAntipatternChecker = scheme.runAction?.diagnosticsOptions.performanceAntipatternCheckerEnabled == false
        let disableGPUValidationMode = scheme.runAction?.diagnosticsOptions.gpuValidationModeEnabled == false
        ...
        return XCScheme.LaunchAction(
            ...
            environmentVariables: environments,
            language: scheme.runAction?.options.language?.identifier,
    ...
    ///     - graphTraverser: Geko graph traverser.
    ///     - rootPath: Path to the project or workspace.
    ///     - generatedProjects: Project paths mapped to generated projects.
