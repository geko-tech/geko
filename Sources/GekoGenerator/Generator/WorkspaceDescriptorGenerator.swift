...
final class WorkspaceDescriptorGenerator: WorkspaceDescriptorGenerating {
    ...
    private let autogeneratedXCTestPlanDescriptorGenerator: AutogeneratedXCTestPlanDescriptorGenerating
    ...
    convenience init(
        ...
        self.init(
            ...
            autogeneratedXCTestPlanDescriptorGenerator: AutogeneratedXCTestPlanDescriptorGenerator(),
            config: config
        )
    }
    ...
    init(
        ...
        autogeneratedXCTestPlanDescriptorGenerator: AutogeneratedXCTestPlanDescriptorGenerating,
        config: Config = .default
    ) {
        ...
        self.config = config
        self.autogeneratedXCTestPlanDescriptorGenerator = autogeneratedXCTestPlanDescriptorGenerator
    }
    ...
    func generate(graphTraverser: GraphTraversing, sideTable: GraphSideTable) throws -> (descriptor: WorkspaceDescriptor, sideEffects: [SideEffectDescriptor]) {
        ...
        let (autogeneratedXCTestPlanDescriptors, generateMetadata) = try autogeneratedXCTestPlanDescriptorGenerator.autogeneratedXCTestPlanDescriptorsAndGenerateMetadata(
            graphTraverser: graphTraverser,
            generatedProjects: generatedProjects,
            sideTable: sideTable
        )
        let generatorPaths = GeneratorPaths(manifestDirectory: graphTraverser.workspace.path)
        let deleteOldTestsPlansSideEffects = try graphTraverser.workspace.generationOptions.autogeneratedTestPlans.map {
            SideEffectDescriptor.file(FileDescriptor(path: try $0.resolvePath(generatorPaths: generatorPaths), state: .absent))
        }
        let createTestPlansSideEffects = try autogeneratedXCTestPlanDescriptors.map {
