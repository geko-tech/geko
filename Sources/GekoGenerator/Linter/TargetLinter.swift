...
class TargetLinter: TargetLinting {
    ...
    private func lintBundleIdentifier(target: Target) -> [LintingIssue] {
        guard var bundleIdentifier = target.bundleId else { return [] }
        ...
        if !bundleIdentifier.unicodeScalars.allSatisfy({ allowed.contains($0) }) {
            let reason =
                "Invalid bundle identifier '\(bundleIdentifier)' for target '\(target.name)'. This string must be a uniform type identifier (UTI) that contains only alphanumeric (A-Z,a-z,0-9), hyphen (-), and period (.) characters."
    ...
    private func lintHasSourceFiles(target: Target) -> [LintingIssue] {
        ...
        let hasNoSources = supportsSources && sources.isEmpty && target.buildableFolders.isEmpty
        ...
        if hasNoSources, hasNoDependencies, hasNoScripts {
            ...
            return [
                LintingIssue(
                    reason:
                        "Target \(target.name) cannot contain sources. \(target.product) targets in one of these destinations doesn't support source files: \(target.destinations.map(\.rawValue).sorted().joined(separator: ", "))",
                    severity: .error
                )
            ]
        }
    ...
    private func validateCoreDataModelVersionsExist(target: Target) -> [LintingIssue] {
        target.coreDataModels.compactMap { coreDataModel -> LintingIssue? in
            guard let currentVersion = coreDataModel.currentVersion else {
                let reason = "Missing default version of the Core Data model at path \(coreDataModel.path.pathString). Field currentVersion must be not nil."
                return LintingIssue(reason: reason, severity: .error)
            }

            let versionFileName = "\(currentVersion).xcdatamodel"
            let versionPath = coreDataModel.path.appending(component: versionFileName)

            if !FileHandler.shared.exists(versionPath) {
                let reason =
                    "The default version of the Core Data model at path \(coreDataModel.path.pathString), \(currentVersion), does not exist. There should be a file at \(versionPath.pathString)"
                return LintingIssue(reason: reason, severity: .error)
            }
            return nil
        }
    ...
    private func lintDeploymentTarget(target: Target) -> [LintingIssue] {
        target.deploymentTargets.configuredVersions.flatMap { platform, version in
            ...
            if !version.matches(pattern: osVersionRegex) {
                let versionFormatIssue = LintingIssue(
                    reason: "The version \"\(version)\" of deployment target for \(target.name) is incorrect",
                    severity: .error
                )
    ...
    private func lintValidSourceFileCodeGenAttributes(target: Target) -> [LintingIssue] {
        ...
        var unsupportedSourceFileAttributes: [AbsolutePath] = []
        for sourceFiles in target.sources {
            guard sourceFiles.codeGen != nil else { continue }

            for source in sourceFiles.paths {
                if !knownSupportedExtensions.contains(source.extension ?? "") {
                    unsupportedSourceFileAttributes.append(source)
                }
            }
        }
        ...
        return unsupportedSourceFileAttributes.map {
            LintingIssue(
                reason:
                    "Target '\(target.name)' has a source file at path \($0) with unsupported `codeGen` attributes. Only \(knownSupportedExtensions.listed()) are known to support this.",
                severity: .warning
            )
        }
    }
