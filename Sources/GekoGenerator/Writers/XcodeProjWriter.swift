...
public final class XcodeProjWriter: XcodeProjWriting {
    ...
    public func write(workspace: WorkspaceDescriptor) throws {
        ...
        try workspace.autogeneratedXCTestPlanDescriptors.forEach {
            try writeTestPlan(testPlanDescriptor: $0)
        }
    ...
    private func writeTestPlan(testPlanDescriptor: AutogeneratedXCTestPlanDescriptor) throws {
        let parent = testPlanDescriptor.path.parentDirectory
        if !FileHandler.shared.exists(parent) {
            try FileHandler.shared.createFolder(parent)
        }
        let data = try testPlanDescriptor.encode()
        try data.write(to: testPlanDescriptor.path.url, options: .atomic)
    }
