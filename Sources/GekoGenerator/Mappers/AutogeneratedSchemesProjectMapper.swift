...
public final class AutogeneratedSchemesProjectMapper: ProjectMapping { // swiftlint:disable:this type_body_length
    ...
    public func map(
        ...
        case .pods:
            ...
            project.schemes = schemes.sorted { $0.name < $1.name }
            return []
        case .notGrouped:
            targetGroups = [:]
        case .none:
        ...
        schemes.append(contentsOf: buildTargets.filter { $0.product == .framework || $0.product == .staticFramework }.map { target in
            Scheme(
                ...
                testAction: .testPlans(
                    project.options.testPlans.compactMap { try? AbsolutePath(validatingAbsolutePath: $0) }
                )
            )
        })
    ...
    private func createDefaultScheme(
        ...
        let runAction = actualRunTarget.map {
            RunAction(
                ...
                options: .init(
                    language: project.options.runLanguage.map { SchemeLanguage(identifier: $0) },
...
extension Product {
    var isExtension: Bool {
        ...
        case .staticFramework, .app, .appClip, .staticLibrary, .dynamicLibrary, .framework, .unitTests, .uiTests, .bundle,
             .commandLineTool, .watch2App, .xpc, .macro, .aggregateTarget:
            return false
        }
    }
}
