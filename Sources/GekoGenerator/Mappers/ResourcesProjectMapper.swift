...
public class ResourcesProjectMapper: ProjectMapping {
    ...
    public func map(
        ...
        for i in 0 ..< project.targets.count {
            let (newTargets, targetSideEffects) = try mapTarget(
                &project.targets[i],
                projectName: project.name,
                project: project
            )
            additionalTargets.append(contentsOf: newTargets)
            sideEffects.append(contentsOf: targetSideEffects)
        }

        project.targets.append(contentsOf: additionalTargets)

        return sideEffects
    }

    public func mapTarget(_ target: inout Target, projectName: String, project: Project) throws -> ([Target], [SideEffectDescriptor]) {
        if target.resources.isEmpty, target.coreDataModels.isEmpty { return ([], []) }
        
        var additionalTargets: [Target] = []
        var sideEffects: [SideEffectDescriptor] = []

        let bundleName = "\(projectName)_\(target.name.sanitizedModuleName)"
        ...
        if !target.supportsResources {
            let resourcesTarget = Target(
                ...
                bundleId: target.bundleId.map { "\($0).generated.resources" },
                deploymentTargets: target.deploymentTargets,
                infoPlist: .extendingDefault(with: [:]),
                resources: .init(resources: target.resources),
            ...
            target.resources = []
            target.copyFiles = []
            target.dependencies.append(.target(name: bundleName, status: .required, condition: .when(target.dependencyPlatformFilters)))
        ...
        if target.sources.containsSwiftFiles {
            ...
            let sourceFile = SourceFiles(paths: [filePath])
            let sideEffect = SideEffectDescriptor.file(.init(path: filePath, contents: data, state: .present))
            target.sources.append(sourceFile)
            sideEffects.append(sideEffect)
        }
        
        if project.isExternal,
           originalTarget.sources.containsObjcFiles,
           originalTarget.resources.containsBundleAccessedResources,
           !originalTarget.supportsResources {
            ...
            let gccPrefixHeader = "$(SRCROOT)/\(headerFilePath.relative(to: project.path).pathString)"
            ...
            let (resourcesAccessorPath, resourcesAccessorData) = synthesizedObjcImplementationFile(bundleName: bundleName, target: target, project: project)
            target.sources.append(SourceFiles(paths: [resourcesAccessorPath]))
            sideEffects.append(SideEffectDescriptor.file(.init(path: resourcesAccessorPath, contents: resourcesAccessorData, state: .present)))
        }

        return (additionalTargets, sideEffects)
    }
    
    private func synthesizedObjcHeaderFile(
        bundleName: String,
        target: Target,
        project: Project
    ) -> (AbsolutePath, Data?) {
        let filePath = synthesizedFilePath(target: target, project: project, fileExtension: "h")
        let content = ResourcesProjectMapper.objcHeaderFileContent(targetName: target.name)
    ...
    private func synthesizedObjcImplementationFile(
        ...
        let content = ResourcesProjectMapper.objcImplementationFileContent(targetName: target.name, bundleName: bundleName)
        return (filePath, content.data(using: .utf8))
    }
    
    private func synthesizedSwiftFile(bundleName: String, target: Target, project: Project) -> (AbsolutePath, Data?) {
        let filePath = project.derivedDirectoryPath(for: target.name)
            .appending(component: Constants.DerivedDirectory.sources)
            .appending(component: "GekoBundle+\(target.name.toValidSwiftIdentifier()).swift")
    ...
    private func synthesizedFilePath(target: Target, project: Project, fileExtension: String) -> AbsolutePath {
        let fileName = "GekoBundle+\(target.name.uppercasingFirst).\(fileExtension)"
        return project.derivedDirectoryPath(for: target.name).appending(components: [Constants.DerivedDirectory.sources, fileName])
    }
    
    // swiftlint:disable:next function_body_length
    static func fileContent(targetName: String, bundleName: String, target: Target, in project: Project) -> String {
        ...
        let publicBundleAccessor: String
        if project.isExternal {
            publicBundleAccessor = ""
        } else if target.sourcesContainsPublicResourceClassName {
            publicBundleAccessor = ""
        } else {
            publicBundleAccessor = publicBundleAccessorString(for: target)
        }
    ...
    static func objcImplementationFileContent(
        ...
        return """
        #import <Foundation/Foundation.h>
        #import "GekoBundle+\(targetName).h"
    ...
    static func publicBundleAccessorString(for target: Target) -> String {
        """
    ...
    static func swiftSPMBundleAccessorString(for target: Target, and bundleName: String) -> String {
        """
    ...
    static func swiftFrameworkBundleAccessorString(for target: Target) -> String {
        """
...
extension [SourceFiles] {
    fileprivate var containsSwiftFiles: Bool {
        contains(where: { $0.paths.contains { $0.path.extension == "swift" }})
    }
    
    fileprivate var containsObjcFiles: Bool {
        contains(where: { $0.paths.contains { $0.path.extension == "m" || $0.path.extension == "mm" }})
    }
}

extension Target {
    fileprivate var sourcesContainsPublicResourceClassName: Bool {
        sources.contains(where: { $0.paths.contains(where: { $0.basename == "\(name)Resources.swift" })})
    }
}

extension [ResourceFileElement] {
    fileprivate var containsBundleAccessedResources: Bool {
        contains(where: { $0.path.extension != "xcprivacy" })
    }
}
