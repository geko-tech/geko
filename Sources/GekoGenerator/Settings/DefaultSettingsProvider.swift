...
enum BuildSettingsError: FatalError {
    ...
    case consistencyIssue(String)

    var description: String {
        switch self {
        case let .invalidValue(value):
            return "Cannot convert \"\(value)\" to SettingValue type"
        case let .consistencyIssue(value):
            return "Consistency issue: build setting \"\(value)\" has multiple values"
        }
    }
...
enum BuildSettingsConstants {
    static let captureVariableInBuildConfigRegex = try! NSRegularExpression(pattern: "\\$[{]\(variableNamePattern)[}]|\\$[(]\(variableNamePattern)[)]")
    static let variableNamePattern = "([_a-zA-Z0-9]+?)"
    static let inheritedKeywords = ["$(inherited)", "${inherited}"]
    static let inheritedVariableValue = "inherited"
    static let inheritedPattern = try! NSRegularExpression(pattern: #"(?-mix:\$\(inherited\)|\$\{inherited\})"#)
    static let mutalRecursionSentinel = SettingValue.string("xcodeproj.mutual_recursion_sentinel")
    fileprivate static let splitBuildSettingToArrayRegex = try! NSRegularExpression(pattern: #"((['\"]?).*?[^\\]\2)(?=( |\z))"#)
}
...
extension BuildSettings {
    ...
    static func anyToSettingValue(_ value: BuildSetting?) -> SettingValue? {
        guard let value else { return nil }
        switch value {
        case let .string(value):
            let splitToArray = splitBuildSettingToArray(value: value)
            guard splitToArray.count > 1 else {
                return .string(value)
            }

            return .array(splitToArray)
        case let .array(value):
            return .array(value)
        }
    }

    static func splitBuildSettingToArray(value: String) -> [String] {
        let regex = BuildSettingsConstants.splitBuildSettingToArrayRegex
        let valueRange = NSRange(value.startIndex ..< value.endIndex, in: value)
        let matches = regex.matches(in: value, range: valueRange)
        return matches
            .compactMap { Range($0.range, in: value).map { String(value[$0]).trimmingCharacters(in: .whitespacesAndNewlines) } }
    }
}
