...
/// Reference generator.
public final class ReferenceGenerator: ReferenceGenerating {
    let outputSettings: PBXOutputSettings
    var references: Set<String> = []

    public init(outputSettings: PBXOutputSettings) {
    ...
    ///
    /// - Parameter proj: project whose objects references will be generated.
    public func generateReferences(proj: PBXProj) throws {
        ...
        try project.projectReferences.forEach { objectReferenceDict in
            guard let projectReference = objectReferenceDict[Xcode.ProjectReference.projectReferenceKey]?.getObject() as? PBXFileReference else { return }
            try generateFileReference(projectReference, identifiers: identifiers)

            guard let productsGroup = objectReferenceDict[Xcode.ProjectReference.productGroupKey]?.getObject() as? PBXGroup else { return }
    ...
    private func generateGroupReferences(_ group: PBXGroup,
        ...
        try group.childrenReferences.forEach { child in
            ...
            } else if let fileSystemSynchronizedRootGroup = childFileElement as? PBXFileSystemSynchronizedRootGroup {
                try generateFileSystemSynchronizedRootGroupReference(fileSystemSynchronizedRootGroup, identifiers: identifiers)
            }
        }
    }
    ...
    ///   - fileReference: file reference instance.
    ///   - identifiers: list of identifiers.
    private func generateFileSystemSynchronizedRootGroupReference(_ group: PBXFileSystemSynchronizedRootGroup,
                                                                  identifiers: [String]) throws {
        var identifiers = identifiers
        if let groupName = group.fileName() {
            identifiers.append(groupName)
        ...
        for exception in group.exceptions ?? [] {
            generateFileSystemSynchronizedExceptionSetReference(
                exception,
    ...
    private func generateFileSystemSynchronizedExceptionSetReference(
        _ exception: PBXFileSystemSynchronizedExceptionSet,
        identifiers: [String]
    ) {
        var identifiers = identifiers
        if let exceptions = membershipExceptions(exception) {
            identifiers.append(contentsOf: exceptions)
        }

        fixReference(for: exception, identifiers: identifiers)
    }

    private func membershipExceptions(_ exception: PBXFileSystemSynchronizedExceptionSet) -> [String]? {
        switch exception {
        case let exception as PBXFileSystemSynchronizedBuildFileExceptionSet:
            exception.membershipExceptions
