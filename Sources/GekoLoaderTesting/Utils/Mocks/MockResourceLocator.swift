...
public final class MockResourceLocator: ResourceLocating {
    ...
    public func projectDescription() throws -> AbsolutePath {
        ...
        return try projectDescriptionStub?() ?? AbsolutePath(validatingAbsolutePath: "/")
    }

    public func cliPath() throws -> AbsolutePath {
        cliPathCount += 1
        return try cliPathStub?() ?? AbsolutePath(validatingAbsolutePath: "/")
    }

    public func embedPath() throws -> AbsolutePath {
        embedPathCount += 1
        return try embedPathStub?() ?? AbsolutePath(validatingAbsolutePath: "/")
    }
}
