...
struct SettingsMapper {
    ...
    func mapSettings() throws -> SettingsDictionary {
        var resolvedSettings = try settingsDictionary()

        for platform in Platform.allCases.sorted(by: { $0.rawValue < $1.rawValue }) {
    ...
    func settingsDictionary(for platform: Platform? = nil) throws -> SettingsDictionary {
    ...
    private func settings(for platformName: String?) throws -> [PackageInfo.Target.TargetBuildSettingDescription.Setting] {
