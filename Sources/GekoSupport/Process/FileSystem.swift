...
private struct LocalFileSystem: FileSystem {
    ...
    var currentWorkingDirectory: AbsolutePath? {
        ...
        // of Foundation.
        return try? AbsolutePath(validatingAbsolutePath: cwdStr)
#else
    ...
    var homeDirectory: AbsolutePath {
        get throws {
            return try AbsolutePath(validatingAbsolutePath: NSHomeDirectory())
        }
    }

    var cachesDirectory: AbsolutePath? {
        return FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first.flatMap { try? AbsolutePath(validatingAbsolutePath: $0.path) }
    }

    var tempDirectory: AbsolutePath {
        get throws {
            let override = ProcessEnv.vars["TMPDIR"] ?? ProcessEnv.vars["TEMP"] ?? ProcessEnv.vars["TMP"]
            if let path = override.flatMap({ try? AbsolutePath(validatingAbsolutePath: $0) }) {
                return path
            }
            return try AbsolutePath(validatingAbsolutePath: NSTemporaryDirectory())
        }
    }
    ...
    func itemReplacementDirectories(for path: AbsolutePath) throws -> [AbsolutePath] {
        ...
        let path = try AbsolutePath(validatingAbsolutePath: result.path)
...
public final class InMemoryFileSystem: FileSystem {
    ...
    public var currentWorkingDirectory: AbsolutePath? {
        return try? AbsolutePath(validatingAbsolutePath: "/")
    }
    ...
    public var homeDirectory: AbsolutePath {
        get throws {
            ...
            return try AbsolutePath(validatingAbsolutePath: "/home/user")
        }
    }
    ...
    public var tempDirectory: AbsolutePath {
        get throws {
            return try AbsolutePath(validatingAbsolutePath: "/tmp")
        }
    }
