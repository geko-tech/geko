...
public final class Process {
    ...
    public static func findExecutable(
        ...
        if let abs = try? AbsolutePath(validatingAbsolutePath: program) {
            return abs
        }
    ...
    public func launch() throws -> WritableByteStream {
        ...
        guard rv == 0 else {
            throw ProcessSystemError.posix_spawn(rv, arguments)
        }
...
//    #endif
    ...
    private func waitUntilExit(_ completion: @escaping (Result<ProcessResult, Swift.Error>) -> Void) {
        ...
        case .outputReady(let stdoutResult, let stderrResult):
            ...
            if result == -1 {
                self.state = .failed(ProcessSystemError.waitpid(errno))
            }
        ...
        loop: while true {
            ...
            case  -1:
                if errno == EINTR {
                    ...
                    error = ProcessSystemError.read(errno)
                    break loop
                }
            case 0:
...
/// Open the given pipe.
private func open(pipe pipeToOpen: inout [Int32]) throws {
//    let rv = TSCLibc.pipe(&pipe)
    let rv = pipe(&pipeToOpen)
    guard rv == 0 else {
        throw ProcessSystemError.pipe(rv)
    }
}
...
private func close(fd: Int32) throws {
    func innerClose(_ fd: inout Int32) throws {
        let rv = close(fd)
        guard rv == 0 else {
            throw ProcessSystemError.close(rv)
        }
    }
    var innerFd = fd
