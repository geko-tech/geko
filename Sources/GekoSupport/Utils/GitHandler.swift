...
public final class GitHandler: GitHandling {
    ...
    public func pull(in path: AbsolutePath?) throws {
        if let path {
            let gitDirectory = path.appending(component: ".git")
            try run(command: "git", "--git-dir", gitDirectory.pathString, "--work-tree", path.pathString, "pull", "--ff-only")
        } else {
            try run(command: "git", "pull")
        }
    }
    ...
    private func run(command: [String]) throws {
        if Environment.shared.isVerbose && !LogOutput.isQuiet {
