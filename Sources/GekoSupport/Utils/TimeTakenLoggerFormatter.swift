...
public protocol TimeTakenLoggerFormatting {
    ...
    func timeTakenMessage(message: String, for timer: ClockTimer) -> Logger.Message
}
...
public class TimeTakenLoggerFormatter: TimeTakenLoggerFormatting {
    ...
    public func timeTakenMessage(message: String, for timer: ClockTimer) -> Logger.Message {
        let time = String(format: "%.3f", timer.stop())
        return "Total time taken for \(message): \(time)s"
    }
}
