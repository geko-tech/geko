...
public protocol DerivedDataLocating {
    func locate(
        for workspacePath: AbsolutePath
    ) throws -> AbsolutePath
}
...
public final class DerivedDataLocator: DerivedDataLocating {
    ...
    public func locate(
        ...
        switch try DeveloperEnvironment.shared.derivedDataDirectory(for: workspacePath) {
        case .default(let path), .custom(let path):
            let hash = try XcodeProjectPathHasher.hashString(for: workspacePath.pathString)
            return path.appending(component: "\(workspacePath.basenameWithoutExt)-\(hash)")
        case .relative(let path):
            // If you use a relative path for a project, the hash in the folder name will not be used
            return path.appending(component: "\(workspacePath.basenameWithoutExt)")
        }
    }
}
