...
public final class BuildGraphInspector: BuildGraphInspecting {
    ...
    public func workspacePath(directory: AbsolutePath) throws -> AbsolutePath? {
        return try FileHandler.shared.glob(directory, patterns: ["*.xcworkspace"], exclude: [])
           .first { path in
                try FileHandler.shared.contentsOfDirectory(path).contains {
                    $0.basename == Constants.gekoGeneratedFileName
                }
           }
    }
}
