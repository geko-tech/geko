...
/// helper files under /Geko/ProjectDescriptionHelpers that can be imported
...
public final class ProjectDescriptionHelpersBuilder: ProjectDescriptionHelpersBuilding {
    ...
    private func buildDefaultHelpers(
        ...
        guard let gekoHelpersDirectory = helpersDirectoryLocator.locate(at: path) else { return nil }
        return try buildHelpers(
            name: Self.defaultHelpersName,
            in: gekoHelpersDirectory,
            projectDescriptionSearchPaths: projectDescriptionSearchPaths,
    ...
    private func buildHelpers(
        ...
        #endif
        let swiftmodulePath = helpersModuleCachePath.appending(component: "\(name).swiftmodule")
        ...
        if FileHandler.shared.exists(modulePath) && FileHandler.shared.exists(swiftmodulePath) {
    ...
    private func createCommand(
        ...
        if files.count > 1 {
            ...
            command.append(contentsOf: ["-j", "\(threadsCount)"])
        }
