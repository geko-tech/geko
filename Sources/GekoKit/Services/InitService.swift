...
enum InitServiceError: FatalError, Equatable {
    ...
    var description: String {
        ...
        case let .templateNotFound(template):
            return "Could not find template \(template). Make sure it exists at Geko/Templates/\(template)"
        case .templateNotProvided:
...
class InitService {
    ...
    func run(
        ...
        if templateName.isGitURL {
            try templateGitLoader.loadTemplate(
                from: templateName,
                closure: { template in
    ...
    private func verifyDirectoryIsEmpty(path: AbsolutePath) throws {
        ...
        let isDirEmpty = try FileHandler.shared.contentsOfDirectory(path).filter({$0.basename != Constants.versionFileName }).isEmpty
        if !isDirEmpty {
