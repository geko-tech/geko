...
public class MockFileUnarchiver: FileUnarchiving {
    ...
    public var invokedUnarchive = false
    public var invokedUnarchiveCount = 0
    public var stubbedUnarchiveError: Error?
    public var stubbedUnarchiveResult: AbsolutePath!

    public func unarchive() throws -> AbsolutePath {
        invokedUnarchive = true
        invokedUnarchiveCount += 1
        if let error = stubbedUnarchiveError {
            throw error
        }
        return stubbedUnarchiveResult
    }
    ...
    public var invokedUntar = false
    public var invokedUntarCount = 0
    public var stubbedUntarError: Error?
    public var stubbedUntarResult: AbsolutePath!

    public func untar(flatten: Bool) throws -> AbsolutePath {
        invokedUntar = true
        invokedUntarCount += 1
        if let error = stubbedUntarError {
            throw error
        }
        return stubbedUntarResult
    }
