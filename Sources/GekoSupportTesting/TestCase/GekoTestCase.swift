...
public final class MockFileHandler: FileHandler {
    ...
    public var stubReadTextFile: ((AbsolutePath) throws -> String)?
    override public func readTextFile(_ path: AbsolutePath) throws -> String {
        guard let stubReadTextFile else {
            return try super.readTextFile(path)
        }
        return try stubReadTextFile(path)
    }
    ...
    public var stubGlob: (([AbsolutePath], [AbsolutePath]) -> [AbsolutePath])?
    override public func glob(_ path: AbsolutePath, glob: String) -> [AbsolutePath] {
        return (try? self.glob([path.appending(RelativePath(validatingRelativePath: glob))], excluding: [], errorLevel: .none, checkFilesExist: false)) ?? []
    }

    override public func glob(_ paths: [AbsolutePath], excluding: [AbsolutePath], errorLevel: FileHandlerGlobErrorLevel, checkFilesExist: Bool) throws -> [AbsolutePath] {
        guard let stubGlob else {
            return try super.glob(paths, excluding: excluding, errorLevel: errorLevel, checkFilesExist: checkFilesExist)
        }
        return stubGlob(paths, excluding)
    }
}

open class GekoTestCase: XCTestCase {
