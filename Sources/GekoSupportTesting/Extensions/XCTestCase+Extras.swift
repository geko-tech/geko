...
extension XCTestCase {
    ...
    public func fixturePath(path: RelativePath) -> AbsolutePath {
        try! AbsolutePath(validatingAbsolutePath: #file) // swiftlint:disable:this force_try
    ...
    public func XCTAssertNoThrows(
        ...
        do {
            ...
            XCTFail("error was thrown: \(closureError)", file: file, line: line)
            return
        }
    }
    ...
    public func XCTAssertEncodableEqualToJson(
        ...
        if let subjectArray = subjectObject as? [Any], let jsonArray = jsonObject as? [Any] {
            ...
            XCTAssertTrue(subjectNSArray.isEqual(to: [Any](jsonNSArray)), errorString, file: file, line: line)
            ...
            XCTAssertTrue(subjectNSDictionary.isEqual(to: jsonDictionary), errorString, file: file, line: line)
        } else {
