...
public enum XcodeBuildArgument: Equatable, CustomStringConvertible {
    ...
    public var arguments: [String] {
        switch self {
        case let .sdk(sdk):
            return ["-sdk", sdk.spm_shellEscaped()]
        case let .configuration(configuration):
            return ["-configuration", configuration.spm_shellEscaped()]
        case let .destination(destination):
            return ["-destination", "\(destination.spm_shellEscaped())"]
        ...
        case let .arch(architecture):
            return ["-arch", architecture.spm_shellEscaped()]
