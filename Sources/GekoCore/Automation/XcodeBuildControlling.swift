...
public enum XcodeBuildControllerCreateXCFrameworkArgument {
    ...
    /**
     An argument that represents a framework path.
     
     xcodebuild -create-xcframework
       -framework Some/Absolute/Path/To/MyFramework_iOS.framework
       -framework Some/Absolute/Path/To/MyFramework_macOS.framework
       -output MyFramework.xcframework -allow-internal-distribution
     */
    case framework(frameworkPath: AbsolutePath)
     ...
     */
    case frameworkWithArchive(archivePath: AbsolutePath, framework: String)
    ...
    public var xcodebuildArguments: [String] {
        func sanitizedPath(_ path: AbsolutePath) -> String {
            ...
            path.pathString.hasPrefix("/var/")
                ? path.pathString.replacingOccurrences(of: "/var/", with: "/private/var/")
                : path.pathString
        }
        switch self {
        case let .framework(frameworkPath):
            return ["-framework", sanitizedPath(frameworkPath)]
        case let .frameworkWithArchive(archivePath, framework):
