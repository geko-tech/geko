...
public final class SimulatorController: SimulatorControlling {
    ...
    public func findAvailableDevices(
        ...
        var maxRuntimeVersions: [String: SimulatorRuntimeVersion] = [:]
        for devicesAndRuntime in devicesAndRuntimes {
            guard let name = devicesAndRuntime.runtime.name.components(separatedBy: " ").first else {
                continue
            }
            if let prevVersion = maxRuntimeVersions[name] {
                if prevVersion < devicesAndRuntime.runtime.version {
                    maxRuntimeVersions[name] = devicesAndRuntime.runtime.version
                }
            } else {
                maxRuntimeVersions[name] = devicesAndRuntime.runtime.version
            }
        ...
        let availableDevices = devicesAndRuntimes
            ...
            .filter { simulatorDeviceAndRuntime in
                ...
                if version == nil, let maxVersion = maxRuntimeVersions[platform.caseValue] {
                    guard simulatorDeviceAndRuntime.runtime.version == maxVersion else { return false }
                }
