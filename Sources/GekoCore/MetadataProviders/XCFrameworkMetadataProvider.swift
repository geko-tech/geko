...
public protocol XCFrameworkMetadataProviding: PrecompiledMetadataProviding {
    ...
    /// Returns path to swiftmodule folder if it is exist
    /// - Parameter xcframeworkPath: Path to the xcframework.
    /// - Parameter library: Framework library to
    func swiftmoduleFolderPath(xcframeworkPath: AbsolutePath, library: XCFrameworkInfoPlist.Library) -> AbsolutePath?
...
public final class XCFrameworkMetadataProvider: PrecompiledMetadataProvider, XCFrameworkMetadataProviding {
    ...
    public func macroPath(xcframeworkPath: AbsolutePath) throws -> AbsolutePath? {
        ...
        return try AbsolutePath(validatingAbsolutePath: macroPath.pathString)
    }
    ...
    public func swiftmoduleFolderPath(xcframeworkPath: AbsolutePath, library: XCFrameworkInfoPlist.Library) -> AbsolutePath? {
        let swiftModuleFolderPath = xcframeworkPath.appending(components: [
            library.identifier,
            library.path.pathString,
            "Modules",
            "\(library.binaryName).swiftmodule"
        ])
