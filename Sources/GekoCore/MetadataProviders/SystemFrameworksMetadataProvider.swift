...
public protocol SystemFrameworkMetadataProviding {
    func loadMetadata(sdkName: String, status: LinkingStatus, platform: Platform, source: SDKSource) throws -> SystemFrameworkMetadata
}
...
public final class SystemFrameworkMetadataProvider: SystemFrameworkMetadataProviding {
    ...
    private func sdkPath(name: String, platform: Platform, type: GekoGraph.SDKType, source: SDKSource) throws -> AbsolutePath {
        ...
        case .developer:
            ...
            let sdkRootPath = try AbsolutePath(validatingAbsolutePath: "/\(xcodeDeveloperSdkRootPath)")
            return sdkRootPath
                .appending(try RelativePath(validating: "Frameworks"))
                .appending(component: name)

        case .system:
            let sdkRootPath = try AbsolutePath(validatingAbsolutePath: "/\(platform.xcodeSdkRootPath)")
            switch type {
            case .framework:
                return sdkRootPath
