...
extension Project {
    ...
    public func derivedDirectoryPath(for targetName: String) -> AbsolutePath {
        if isExternal, path.pathString.contains("\(Constants.DependenciesDirectory.packageBuildDirectoryName)/\(Constants.DependenciesDirectory.spmCheckouts)") {
            return path
            ...
            return path.appending(component: Constants.DerivedDirectory.name)
        }
    }
}
