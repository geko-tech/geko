...
public protocol GraphTraversing {
    ...
    /// Given a project directory and target name, it returns **closest** target dependencies.
    /// This means that it returns direct target dependencies plus transitive target dependencies accessible through precompiled frameworks.
    /// instead
    /// - Parameters:
    ///   - path: Path to the directory that contains the target's project.
    ///   - name: Target name.
    ...
    /// Given a project directory and a target name, it returns a list of static precompiled xcframework dependencies
    ///
    /// - Parameters:
    ///   - path: Path to the directory that contains the project.
    ///   - name: Target name
    func staticXCFrameworkDependencies(path: AbsolutePath, name: String) -> Set<GraphDependencyReference>
    ...
    /// Given a project directory and a target name, it returns the list of folders with Swift modules that should be exposed to
    /// the target.
    /// - Parameters:
    ///   - path: Path to the directory that contains the project.
    ///   - name: Target name.
    ...
    /// It returns a set containing the external dependencies that are declared in Dependencies.swift but not added to the targets 
    /// - Returns: The list of dependencies.
    ...
    /// they don't really support. To prevent this from causing compilation issues, Geko cascades
