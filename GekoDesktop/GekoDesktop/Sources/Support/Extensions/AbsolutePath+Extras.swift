...
// TODO: Replace to Geko ToolSupport lib when become available

let systemGlob = Darwin.glob

enum GlobError: FatalError, Equatable {
    case nonExistentDirectory(InvalidGlob)

    public var type: FatalErrorType { .abort }
    public var shouldShowAlert: Bool { true }
...
extension AbsolutePath {
    ...
    public func throwingGlob(_ pattern: String) throws -> [AbsolutePath] {
        ...
        if globPath.isGlobComponent {
            ...
            if !FileManager.default.isFolder(pathUpToLastNonGlob) {
...
extension AbsolutePath {
    public init(url: URL) {
        self.init(stringLiteral: url.absoluteString.replacingOccurrences(of: "file://", with: ""))
    }
}
