...
extension NSColor {
    func getTerminalColor () -> SwiftTermColor {
        guard let color = self.usingColorSpace(.deviceRGB) else {
            return SwiftTermColor.defaultForeground
        }
        
        var red: CGFloat = 0.0, green: CGFloat = 0.0, blue: CGFloat = 0.0, alpha: CGFloat = 1.0
        color.getRed(&red, green: &green, blue: &blue, alpha: &alpha)
        return SwiftTermColor(red: UInt16(red*65535), green: UInt16(green*65535), blue: UInt16(blue*65535))
    }
    ...
    static func make (color: SwiftTermColor) -> NSColor
    {
