...
final class ProjectEditorTests: GekoUnitTestCase {
    ...
    func test_edit() throws {
        ...
        let gekoPath = try AbsolutePath(validating: ProcessInfo.processInfo.arguments.first!)
        let configPath = directory.appending(components: "Geko", "Config.swift")
        let dependenciesPath = directory.appending(components: "Geko", "Dependencies.swift")
        let packageManifestPath = directory.appending(components: "Geko", "Package.swift")
        ...
        try FileHandler.shared.write(
            ...
            B.swift
            """,
            path: directory.appending(component: ".gekoignore"),
            atomically: true
        )
        ...
        XCTAssertEqual(mapArgs?.gekoPath, gekoPath)
        XCTAssertEqual(mapArgs?.helpers.sorted(), helpers.sorted())
        XCTAssertEqual(mapArgs?.sourceRootPath, directory)
        XCTAssertEqual(mapArgs?.projectDescriptionSearchPath.frameworkSearchPath, projectDescriptionPath.parentDirectory)
    ...
    func test_edit_project_with_remote_plugin() throws {
        ...
        XCTAssertEqual(mapArgs?.sourceRootPath, directory)
        XCTAssertEqual(mapArgs?.projectDescriptionSearchPath.frameworkSearchPath, projectDescriptionPath.parentDirectory)
        XCTAssertEqual(mapArgs?.editablePluginManifests.map(\.name), ["RemotePlugin"])
        XCTAssertEqual(mapArgs?.pluginProjectDescriptionHelpersModule, [])
    }
}
