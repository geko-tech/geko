...
final class ProjectEditorMapperTests: GekoUnitTestCase {
    ...
    func test_edit_when_there_are_helpers_and_setup_and_config_and_dependencies_and_tasks_and_plugins() throws {
        ...
        let configPath = sourceRootPath.appending(components: Constants.gekoDirectoryName, "Config.swift")
        let dependenciesPath = sourceRootPath.appending(components: Constants.gekoDirectoryName, "Dependencies.swift")
        let packageManifestPath = sourceRootPath.appending(components: Constants.gekoDirectoryName, "Package.swift")
        ...
        let projectDescriptionSearchPaths = ProjectDescriptionSearchPaths.paths(for: projectDescriptionPath)
        let gekoPath = try AbsolutePath(validating: "/usr/bin/foo/bar/geko")
        ...
        ].map { EditablePluginManifest(name: $0.basename, path: $0, location: .local) }
        ...
        XCTAssertEqual(
            ...
            expectedSettings(includePaths: [projectDescriptionPath.parentDirectory])
        )
        XCTAssertEqual(manifestsTarget.sources.flatMap(\.paths), projectManifestPaths)
        XCTAssertEqual(manifestsTarget.filesGroup, projectsGroup)
        XCTAssertEqual(
            Set(manifestsTarget.dependencies),
            Set([
        ...
        XCTAssertEqual(helpersTarget.sources.flatMap(\.paths), helperPaths)
        ...
        XCTAssertEqual(templatesTarget.sources.flatMap(\.paths), templates)
        ...
        XCTAssertEqual(
            Set(templatesTarget.dependencies),
            Set([
                .target(name: "ProjectDescriptionHelpers")
            ]))
        ...
        XCTAssertEqual(stencilsTarget.sources.flatMap(\.paths), stencils)
        XCTAssertEqual(stencilsTarget.filesGroup, projectsGroup)
        XCTAssertEqual(
            Set(stencilsTarget.dependencies),
            Set([
                .target(name: "ProjectDescriptionHelpers")
            ]))
        ...
        XCTAssertEqual(configTarget.sources.flatMap(\.paths), [configPath])
        ...
        XCTAssertEqual(dependenciesTarget.sources.flatMap(\.paths), [dependenciesPath])
        XCTAssertEqual(dependenciesTarget.filesGroup, projectsGroup)
        XCTAssertEqual(
            Set(dependenciesTarget.dependencies),
            Set([
                .target(name: "ProjectDescriptionHelpers"),
                .target(name: "PluginTwo"),
                .target(name: "PluginThree"),
        ...
        XCTAssertEqual(packagesTarget.sources.flatMap(\.paths), [packageManifestPath])
        ...
        XCTAssertEqual(runAction.filePath, gekoPath)
    ...
    func test_geko_edit_with_more_than_one_manifest() throws {
        // Given
        let sourceRootPath = try temporaryPath()
        let configPath = sourceRootPath.appending(components: Constants.gekoDirectoryName, "Config.swift")
        let dependenciesPath = sourceRootPath.appending(components: Constants.gekoDirectoryName, "Dependencies.swift")
        ...
        XCTAssertEqual(manifestOneTarget.sources.flatMap(\.paths), [try XCTUnwrap(projectManifestPaths.last)])
        ...
        XCTAssertEqual(manifestTwoTarget.sources.flatMap(\.paths), [try XCTUnwrap(projectManifestPaths.first)])
    ...
    func test_geko_edit_with_one_plugin_no_projects() throws {
        let sourceRootPath = try temporaryPath()
        ...
        let editablePluginManifests = pluginManifestPaths.map {
            EditablePluginManifest(name: $0.parentDirectory.basename, path: $0.parentDirectory, location: .local)
        }
        let helperPaths: [AbsolutePath] = []
        let templates: [AbsolutePath] = []
        let stencils: [AbsolutePath] = []
        ...
        XCTAssertEqual(pluginTarget.sources.flatMap(\.paths), pluginManifestPaths)
        ...
        XCTAssertEqual(schemes.map(\.name).sorted(), [pluginTarget.name, "LocalPlugins"].sorted())
    ...
    func test_geko_edit_with_more_than_one_plugin_no_projects() throws {
        let sourceRootPath = try temporaryPath()
        ...
        XCTAssertEqual(firstPluginTarget.sources.flatMap(\.paths), [pluginManifestPaths[0]])
        ...
        XCTAssertEqual(secondPluginTarget.sources.flatMap(\.paths), [pluginManifestPaths[1]])
        ...
        XCTAssertEqual(
            schemes.map(\.name),
            [firstPluginTarget.name, secondPluginTarget.name, "LocalPlugins"].sorted()
        )
    ...
    func test_geko_edit_plugin_only_takes_required_sources() throws {
        // Given
        let sourceRootPath = try temporaryPath()
        let pluginManifestPath = sourceRootPath.appending(component: "Plugin.swift")
        let editablePluginManifests = [
            EditablePluginManifest(name: pluginManifestPath.parentDirectory.basename, path: pluginManifestPath.parentDirectory, location: .local)
        ]
        let helperPaths: [AbsolutePath] = []
        let templates: [AbsolutePath] = []
        let stencils: [AbsolutePath] = []
        ...
        // Then
        let project = try XCTUnwrap(graph.projects.values.first)
        var pluginTarget = try XCTUnwrap(project.targets.first)
        pluginTarget.sources[0].paths.sort()

        XCTAssertEqual(
            pluginTarget.sources,
            [SourceFiles(paths: [pluginManifestPath] + helperSources + templateSources)]
        )
    }

    func test_geko_edit_project_with_plugin() throws {
        // Given
        let sourceRootPath = try temporaryPath()
        let projectManifestPaths = [sourceRootPath].map { $0.appending(component: "Project.swift") }
        let helperPaths: [AbsolutePath] = [sourceRootPath].map { $0.appending(components: "geko", "ProjectDescriptionHelpers") }
        let templates: [AbsolutePath] = []
        let stencils: [AbsolutePath] = []
        ...
        let pluginsProjectName = "LocalPlugins"
        ...
        let localPlugin = EditablePluginManifest(name: "ALocalPlugin", path: sourceRootPath.appending(component: "ALocalPlugin"), location: .local)
        ...
        XCTAssertEqual(localPluginTarget.sources.flatMap(\.paths).first?.parentDirectory, localPlugin.path)
        ...
        XCTAssertEqual(helpersTarget.destinations, .macOS)
        XCTAssertEqual(helpersTarget.product, .staticFramework)
        XCTAssertEqual(helpersTarget.sources.flatMap(\.paths).first, helperPaths.first)
        XCTAssertEqual(helpersTarget.filesGroup, projectsGroup)
        // Helpers can depend on local editable plugins
        XCTAssertEqual(
            helpersTarget.dependencies,
            [
        ...
        XCTAssertEqual(manifestsRunAction.filePath, gekoPath)
        XCTAssertEqual(
