...
final class MockGenerator: Generating {
    ...
    var invokedLoadWithSideEffectsPath: AbsolutePath?
    var loadWithSideEffectsStub: ((AbsolutePath) throws -> (Graph, GraphSideTable, [SideEffectDescriptor]))?
    func loadWithSideEffects(
        path: AbsolutePath
    ) async throws -> (GekoGraph.Graph, GraphSideTable, [SideEffectDescriptor]) {
        invokedLoadParameterPath = path
        if let loadWithSideEffectsStub {
            return try loadWithSideEffectsStub(path)
        } else {
            return (Graph.test(), GraphSideTable(), [])
        }
    }
