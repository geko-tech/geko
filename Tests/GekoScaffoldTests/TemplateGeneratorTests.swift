...
final class TemplateGeneratorTests: GekoTestCase {
    ...
    func test_files_are_generated() throws {
        ...
        let items: [Template.Item] = [
            Template.Item(path: "a", contents: .string("aContent")),
            Template.Item(path: "b", contents: .string("bContent")),
            Template.Item(path: ".gitkeep", contents: .string("")),
        ]

        let template = Template.test(items: items)
        let destinationPath = try temporaryPath()
        let expectedFiles: [(AbsolutePath, String)] = try items.compactMap {
            let content: String
            switch $0.contents {
            ...
            return (destinationPath.appending(try RelativePath(validatingRelativePath: $0.path)), content)
        }
    ...
    func test_files_are_generated_with_attributes() throws {
        ...
        let items = [
            Template.Item(path: "{{ name }}", contents: .string("{{ contentName }}")),
            Template.Item(
