...
final class GraphTraverserTests: GekoUnitTestCase {
    ...
    func test_closestTargetDependencies_returnsAllTargets() {
        // Given
        // Project A: A1 -> A2
        //               -> (Project B) B1
        //               -> Framework C1
        // Project B: B1
        // Framework C1  -> (Project D) D1
        // Project D: D1
        let projectA = Project.test(path: "/ProjectA", name: "ProjectA")
        let projectB = Project.test(path: "/ProjectB", name: "ProjectB")
        let projectD = Project.test(path: "/ProjectD", name: "ProjectD")
        let a1 = Target.test(name: "A1")
        let a2 = Target.test(name: "A2")
        let b1 = Target.test(name: "B1")
        let c1 = GraphDependency.testFramework()
        let d1 = Target.test(name: "D1")
        let dependencies: [GraphDependency: Set<GraphDependency>] = [
            .target(name: a1.name, path: projectA.path): Set([
                .target(name: a2.name, path: projectA.path),
                .target(name: b1.name, path: projectB.path),
            ...
            c1: Set([
                .target(name: d1.name, path: projectD.path),
            ])
        ]
        let targets: [AbsolutePath: [String: Target]] = [
            projectA.path: [
        ...
        // When
        let got = subject.closestTargetDependencies(path: projectA.path, name: a1.name).sorted()
        ...
        XCTAssertEqual(
            ...
            [
                GraphTargetReference(target: GraphTarget(path: projectA.path, target: a2, project: projectA)),
                GraphTargetReference(target: GraphTarget(path: projectB.path, target: b1, project: projectB)),
                GraphTargetReference(target: GraphTarget(path: projectD.path, target: d1, project: projectD)),
            ]
        )
    }
    ...
    func test_resourceBundleDependencies_precompiledDynamicFramework() {
        // Given

        // App -> Dynamic Framework (Precompiled) ------> Static Framework A -> Bundle A
        ...
        let dynamicFramework = GraphDependency.testFramework(
            path: "/path/cache/CachedStaticFrameworkA.framework",
            linking: .dynamic
        )
    ...
    func test_resourceBundleDependencies_forDynamic_staticDepsPrecompiled() {
        // Given
        
        // App -> Dynamic Framework ------> Static Framework A -> Bundle A
        ...
        let dynamicFramework = Target.test(name: "Dynamic", product: .framework)
        let dynamicFrameworkProject = Project.test(path: "/path/modules/DynamicFramework", targets: [dynamicFramework])
    ...
    func test_resourceBundleDependencies_when_app_depends_on_spm_static_framework_with_resources_via_dynamic_framework() {
        // Given
        // App -> DynamicFramework -> StaticLibrary (SPM) -> Bundle
        let app = Target.test(name: "App", product: .app)
        let dynamicFramework = Target.test(name: "DynamicFramework", product: .framework)
        let staticLibrary = Target.test(name: "StaticLibrary", product: .staticLibrary)
        let bundle = Target.test(name: "Bundle", product: .bundle)
        let project = Project.test(targets: [app, dynamicFramework])
        let spmProject = Project.test(
            path: try! AbsolutePath(validating: "/SpmProject"),
        ...
        let dependencies: [GraphDependency: Set<GraphDependency>] = [
            ...
            .target(name: dynamicFramework.name, path: project.path): Set([.target(
                ...
                path: spmProject.path
            )]),
            .target(name: staticLibrary.name, path: spmProject.path): Set([.target(
                name: bundle.name,
                path: spmProject.path
            )]),
            .target(name: bundle.name, path: spmProject.path): Set([]),
        ]
        ...
        let graph = Graph.test(
            ...
            projects: [
                project.path: project,
                spmProject.path: spmProject
            ],
            targets: [
                project.path: [app.name: app, dynamicFramework.name: dynamicFramework],
                spmProject.path: [staticLibrary.name: staticLibrary, bundle.name: bundle],
            ],
            dependencies: dependencies
        )
        let subject = GraphTraverser(graph: graph)
    ...
    func test_embeddableFrameworks_when_macroExecutableInBetween() throws {
        /**
         ...
         Having a macro executable that links dynamic dependencies is an scenario that Geko might support in the future.
    ...
    func test_linkableDependencies_whenDynamicDependencyTransitivelyUsesStaticDependency() throws {
        // Given
        let app = Target.test(name: "App", product: .app)
        let dynamicFramework = Target.test(name: "DynamicFramework", product: .framework)
        let staticFramework = Target.test(name: "StaticFramework", product: .staticFramework)
    ...
    func test_linkableDependencies_whenStaticDependencyIsReplacedWithTransitiveDynamicFrameworkWhichLinksThatStaticDependency() throws {
        // Given
        let app = Target.test(name: "App", product: .app)
        let dynamicFramework = Target.test(name: "DynamicFramework", product: .framework)
        let staticFramework = Target.test(name: "StaticFramework", product: .staticFramework)
        let dynamicFramework2 = Target.test(name: "DynamicFramework2", product: .framework)
        let staticFramework2 = Target.test(name: "StaticFramework2", product: .staticFramework)
        let project = Project.test(targets: [app, dynamicFramework, staticFramework, dynamicFramework2, staticFramework2])
        ...
        let dependencies: [GraphDependency: Set<GraphDependency>] = [
            .target(name: app.name, path: project.path): [
                .target(name: dynamicFramework.name, path: project.path),
                .target(name: staticFramework.name, path: project.path),
                .target(name: staticFramework2.name, path: project.path),
            ],
            .target(name: dynamicFramework.name, path: project.path): [
                .target(name: staticFramework.name, path: project.path)
            ],
            .target(name: staticFramework.name, path: project.path): [
                .target(name: dynamicFramework2.name, path: project.path)
            ],
            .target(name: dynamicFramework2.name, path: project.path): [
                .target(name: staticFramework2.name, path: project.path)
            ],
    ...
    func test_linkableDependencies_whenStaticDependencyTransitivelyUsedDynamicDependency() throws {
        // Given
        let app = Target.test(name: "App", product: .app)
        let staticFramework = Target.test(name: "StaticFramework", product: .staticFramework)
        let dynamicFramework = Target.test(name: "DynamicFramework", product: .framework)
        let project = Project.test(targets: [app, staticFramework, dynamicFramework])
    ...
    func test_linkableDependencies_transitiveSDKDependenciesStatic() throws {
        ...
        let staticFrameworkA = Target.test(
            ...
            product: .staticFramework,
            dependencies: [.sdk(name: "some.framework", type: .framework, status: .optional)]
        )
    ...
    func test_linkableDependencies_transitiveSDKDependenciesImmediateDependencies() throws {
        ...
        let staticFramework = Target.test(
            ...
            product: .staticFramework,
            dependencies: [
                .sdk(name: "thingone.framework", type: .framework, status: .optional),
                .sdk(name: "thingtwo.framework", type: .framework, status: .required),
            ]
        )
    ...
    func test_linkableDependencies_NoTransitiveSDKDependenciesForStaticFrameworks() throws {
        ...
        let staticFrameworkA = Target.test(
            name: "StaticFrameworkA",
            product: .staticFramework,
            dependencies: [.sdk(name: "ThingOne.framework", type: .framework, status: .optional)]
        )
        let staticFrameworkB = Target.test(
            name: "StaticFrameworkB",
            product: .staticFramework,
            dependencies: [.sdk(name: "ThingTwo.framework", type: .framework, status: .optional)]
        )
    ...
    func test_linkableDependencies_whenHostedTestTarget_withAppDependingOnDynamicFramework() throws {
        // Given
        let dynamicFramework = Target.test(
            name: "DynamicFramework",
            product: .framework
        ...
        // Then
        // dynamic dependencies of hosted app are not present in the app host
        XCTAssertEqual(got, [])
    }

    func test_linkableDependencies_whenUiTestTarget_withAppDependingOnDynamicFramework() throws {
        // Given
        let dynamicFramework = Target.test(
            name: "DynamicFramework",
            product: .framework
        )
        let dynamicPrecompiledFramework = GraphDependency.testFramework(linking: .dynamic)

        let app = Target.test(name: "App", product: .app)
        let tests = Target.test(name: "UITests", product: .uiTests)
        let project = Project.test(path: "/path/a")
        ...
        // Then
        // dynamic dependencies of hosted app are not present in the ui tests
        XCTAssertEqual(got, [])
    }

    func test_linkableDependencies_whenUiTestTargetAndDynamicDependency_withSharedStaticDependency() throws {
        // Given
        let dynamicFramework = Target.test(
            name: "DynamicFramework",
            product: .framework
        ...
        // Then
        // static dependency should still be present
        XCTAssertEqual(got, [.product(target: "StaticFramework", productName: "StaticFramework.framework")])
    }

    func test_linkableDependencies_whenUnitTestTargetAndDynamicDependency_withSharedStaticDependency() throws {
        // Given
        let dynamicFramework = Target.test(
            name: "DynamicFramework",
            product: .framework
        ...
        let app = Target.test(name: "App", product: .app)
        let tests = Target.test(name: "UITests", product: .unitTests)
        let project = Project.test(path: "/path/a")
        ...
        // Then
        // static dependency should be replaced with dynamic dependency with incapsulated static dependency
        XCTAssertEqual(got, [.product(target: "DynamicFramework", productName: "DynamicFramework.framework")])
    }
