...
final class GraphLoaderTests: GekoUnitTestCase {
    ...
    func test_loadWorkspace_sdkDependency() throws {
        // Given
        let targetA = Target.test(name: "A", dependencies: [.sdk(name: "libc++.tbd", type: .library, status: .required)])
        let targetB = Target.test(name: "B", dependencies: [.sdk(name: "SwiftUI.framework", type: .framework, status: .optional)])
    ...
    func test_loadWorkspace_localDependency() throws {
        // Given
        let targetA = Target.test(name: "A", dependencies: [.local(name: "LocalDep")])
        let targetB = Target.test(name: "LocalDep")
        let projectA = Project.test(path: "/A", name: "A", targets: [targetA])
        let projectB = Project.test(path: "/B", name: "B", targets: [targetB])
        let workspace = Workspace.test(path: "/", name: "Workspace", projects: ["/A", "/B"])
        let subject = makeSubject()
    ...
    func test_loadWorkspace_localDependency_notFound() throws {
        // Given
        let targetA = Target.test(name: "A", dependencies: [.local(name: "UnknownDep")])
        let targetB = Target.test(name: "LocalDep")
        let projectA = Project.test(path: "/A", name: "A", targets: [targetA])
        let projectB = Project.test(path: "/B", name: "B", targets: [targetB])
        let workspace = Workspace.test(path: "/", name: "Workspace", projects: ["/A", "/B"])
        ...
        // When
        XCTAssertThrowsError(try subject.loadWorkspace(
            workspace: workspace,
            projects: [
                projectA,
                projectB,
            ...
            // Then
            XCTAssertEqual(
                (error as? GraphLoadingError),
                GraphLoadingError.unexpected(
                    "Local dependency `UnknownDep` is not resolved. Cannot find local target with name `UnknownDep`"
                )
            )
        }
    }
