...
final class XCFrameworkMetadataProviderTests: GekoTestCase {
    ...
    func test_libraries_when_frameworkIsPresent() throws {
        ...
        XCTAssertEqual(infoPlist.libraries, [
            .init(
                ...
                architectures: [.x8664],
                platform: .ios,
                platformVariant: XCFrameworkInfoPlist.Library.PlatformVariant.simulator
            ),
            .init(
...
extension XCFrameworkInfoPlist {
    fileprivate init(libraries: [Library]) {
        self.init(
            libraries: libraries,
            searchPathPlatforms: try! XCFrameworkInfoPlist.searchPathPlatforms(from: libraries)
        )
    }
}
