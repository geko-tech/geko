...
final class SimulatorControllerTests: GekoUnitTestCase {
    ...
    func test_findAvailableDevice_should_findDeviceWithMaxVersion_when_multiplePlatforms() async throws {
        // Given
        let devicesAndRuntimes =
            createSystemStubs(
                devices: true,
                ...
                platformsAndVersions: [.iOS: [
                    ...
                    .init(major: 10, minor: 0)
                ], .visionOS: [
                    .init(major: 2, minor: 0)
                ], .watchOS: [
                    .init(major: 12, minor: 0)
                ]]
            )
        let expectedIOSDeviceAndRuntime = try XCTUnwrap(devicesAndRuntimes.first(where: { $0.runtime.version == "17.0" }))
        let expectedTVOSDeviceAndRuntime = try XCTUnwrap(devicesAndRuntimes.first(where: { $0.runtime.version == "10.0" }))
        let expectedVisionOSDeviceAndRuntime = try XCTUnwrap(devicesAndRuntimes.first(where: { $0.runtime.version == "2.0" }))
        let expectedWatchOSDeviceAndRuntime = try XCTUnwrap(devicesAndRuntimes.first(where: { $0.runtime.version == "12.0" }))

        // When
        let iOSdevice = try await subject.findAvailableDevice(
            platform: .iOS,
            version: nil,
            minVersion: nil,
            deviceName: nil
        )
        let tvOSdevice = try await subject.findAvailableDevice(
            platform: .tvOS,
            version: nil,
            minVersion: nil,
            deviceName: nil
        )
        let visionOSdevice = try await subject.findAvailableDevice(
            platform: .visionOS,
            version: nil,
            minVersion: nil,
            deviceName: nil
        )
        let watchOSdevice = try await subject.findAvailableDevice(
            platform: .watchOS,
            version: nil,
            minVersion: nil,
            deviceName: nil
        ...
        // Then
        XCTAssertEqual(iOSdevice, expectedIOSDeviceAndRuntime)
        XCTAssertEqual(tvOSdevice, expectedTVOSDeviceAndRuntime)
        XCTAssertEqual(visionOSdevice, expectedVisionOSDeviceAndRuntime)
        XCTAssertEqual(watchOSdevice, expectedWatchOSDeviceAndRuntime)
    }
    
    func test_findAvailableDevice_should_notFindDevice_when_multiplePlatforms() async throws {
        // Given
        let devicesAndRuntimes =
            createSystemStubs(
                devices: true,
        ...
        let expectedTVOSDeviceAndRuntime = try XCTUnwrap(devicesAndRuntimes.first(where: { $0.runtime.version == "10.0" }))
        let expectedVisionOSDeviceAndRuntime = try XCTUnwrap(devicesAndRuntimes.first(where: { $0.runtime.version == "2.0" }))
        let expectedWatchOSDeviceAndRuntime = devicesAndRuntimes.first(where: { $0.runtime.version == "12.0" })
        ...
        XCTAssertEqual(tvOSdevice, expectedTVOSDeviceAndRuntime)
        XCTAssertEqual(visionOSdevice, expectedVisionOSDeviceAndRuntime)
        XCTAssertNil(expectedWatchOSDeviceAndRuntime)
    }
    ...
    private func createSystemStubs(
        devices: Bool,
        runtimes: Bool,
        platformsAndVersions: [Platform: [SimulatorRuntimeVersion]] = [.iOS: [.init(major: 14, minor: 4)]]
    ) -> [SimulatorDeviceAndRuntime] {
        let stubs = createSimulatorDevicesAndRuntimes(platformsAndVersions: platformsAndVersions)
    ...
    private func createSimulatorDevicesAndRuntimes(platformsAndVersions: [Platform: [SimulatorRuntimeVersion]]) -> (
        ...
        let runtimesJsonResponse: String = {
            var runtimesJsons: [String] = []
            for platformWithVersions in platformsAndVersions {
                for version in platformWithVersions.value {
                    ...
                    switch platformWithVersions.key {
                    case .iOS:
                        identifier = "com.apple.CoreSimulator.SimRuntime.iOS-\(major)-\(minor)"
                        runtimeName = "iOS \(major).\(minor)"
                    case .macOS:
                        identifier = "com.apple.CoreSimulator.SimRuntime.macOS-\(major)-\(minor)"
                        runtimeName = "macOS \(major).\(minor)"
                    case .tvOS:
                        identifier = "com.apple.CoreSimulator.SimRuntime.tvOS-\(major)-\(minor)"
                        runtimeName = "tvOS \(major).\(minor)"
                    case .watchOS:
                        identifier = "com.apple.CoreSimulator.SimRuntime.watchOS-\(major)-\(minor)"
                        runtimeName = "watchOS \(major).\(minor)"
                    case .visionOS:
                        identifier = "com.apple.CoreSimulator.SimRuntime.xrOS-\(major)-\(minor)"
                        runtimeName = "visionOS \(major).\(minor)"
                    }
            ...
            return """
            { "runtimes" : [ \(runtimesJsons.joined(separator: ",")) ]}
            """
        }()

        var devices: [SimulatorDevice] = []
        let devicesJsonResponse: String = {
            var devicesJsons: [String] = []
            for platformWithVersions in platformsAndVersions {
                for version in platformWithVersions.value {
                    let major = version.major
                    let minor = version.minor ?? .zero
                    ...
                    case .iOS:
                        ...
                        runTimeIdentifier = "com.apple.CoreSimulator.SimRuntime.iOS-\(major)-\(minor)"
                    case .macOS:
                        /// There is no emulator for MacOS.
                        continue
                    case .tvOS:
                        deviceTypeIdentifier = "com.apple.CoreSimulator.SimDeviceType.Apple-TV"
                        deviceName = "Apple TV"
                        runTimeIdentifier = "com.apple.CoreSimulator.SimRuntime.tvOS-\(major)-\(minor)"
                    case .watchOS:
                        deviceTypeIdentifier = "com.apple.CoreSimulator.SimDeviceType.Apple-Watch-Series-6-44mm"
                        deviceName = "Apple Watch Series 6 (44mm)"
                        runTimeIdentifier = "com.apple.CoreSimulator.SimRuntime.watchOS-\(major)-\(minor)"
                    case .visionOS:
                        deviceTypeIdentifier = "com.apple.CoreSimulator.SimDeviceType.Apple-Vision-Pro"
                        deviceName = "Apple Vision Pro"
                        runTimeIdentifier = "com.apple.CoreSimulator.SimRuntime.xrOS-\(major)-\(minor)"
                    }
            ...
            return """
            { "devices" : { \(devicesJsons.joined(separator: ",")) }}
            """
        }()
