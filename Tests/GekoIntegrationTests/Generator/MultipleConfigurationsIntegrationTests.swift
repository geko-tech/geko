...
final class MultipleConfigurationsIntegrationTests: GekoUnitTestCase {
    override func setUp() {
        super.setUp()
        do {
    ...
    private func generateWorkspace(projectSettings: Settings, targetSettings: Settings?) throws {
        ...
        let (descriptor, _) = try subject.generateWorkspace(graphTraverser: graphTraverser, sideTable: GraphSideTable())
    ...
    private func createAppTarget(settings: Settings?) throws -> Target {
        let sourceFile = try pathTo("App/Sources/AppDelegate.swift")
        return Target(
            name: "AppTarget",
            destinations: .iOS,
            product: .app,
            ...
            sources: [SourceFiles(paths: [sourceFile])],
            settings: settings,
