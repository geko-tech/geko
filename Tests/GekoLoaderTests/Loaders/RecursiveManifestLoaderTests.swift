...
final class RecursiveManifestLoaderTests: GekoUnitTestCase {
    ...
    func test_loadWorkspace() throws {
        // Given
        let workspace = Workspace.manifestTest(
            name: "Workspace",
            projects: [
                "A",
                "B",
    ...
    private func createProject(
        ...
        let targets: [Target] = targets.map {
            Target.manifestTest(name: $0.key, dependencies: $0.value)
        }
        return .manifestTest(name: name, targets: targets)
    }
    ...
    private func stub(
        manifest: Project,
        at stubPath: AbsolutePath
    ) throws {
        if stubPath.isAbsolute {
            projectManifests[stubPath] = manifest
        } else {
            let relativePath = stubPath
    ...
    private func stub(
        ...
        if stubPath.isAbsolute {
            workspaceManifests[stubPath] = manifest
        } else {
            let relativePath = stubPath
            let manifestPath = path
                .appending(relativePath)
