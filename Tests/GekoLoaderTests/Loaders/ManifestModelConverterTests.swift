...
class ManifestModelConverterTests: GekoUnitTestCase {
    ...
    func test_loadProject() throws {
        ...
        let manifest = ProjectManifest.manifestTest(name: "SomeProject")
    ...
    func test_loadProject_withTargets() throws {
        ...
        let targetA = TargetManifest.manifestTest(name: "A", sources: [], resources: [])
        let targetB = TargetManifest.manifestTest(name: "B", sources: [], resources: [])
        let manifest = ProjectManifest.manifestTest(
            name: "Project",
            targets: [
                targetA,
                targetB,
    ...
    func test_loadProject_withAdditionalFiles() throws {
        ...
        // Then
        XCTAssertEqual(model.additionalFiles, [try .glob(pattern: AbsolutePath(validatingAbsolutePath: "\(temporaryPath)/Documentation/**/*.md"))])

        try model.resolveGlobs(checkFilesExist: true)
    ...
    func test_loadWorkspace() throws {
        // Given
        let temporaryPath = try temporaryPath()
        let manifest = WorkspaceManifest.manifestTest(name: "SomeWorkspace")
        let manifestLoader = makeManifestLoader(with: [
            temporaryPath: manifest,
        ])
        let subject = makeSubject(with: manifestLoader)
    ...
    func test_loadWorkspace_withProjects() throws {
        ...
        let manifest = WorkspaceManifest.manifestTest(name: "SomeWorkspace", projects: ["A", "B"])
        let manifests = [
            temporaryPath: manifest,
        ]
    ...
    func test_loadWorkspace_withAdditionalFiles() throws {
        ...
        let manifest = WorkspaceManifest.manifestTest(
            name: "SomeWorkspace",
            projects: [],
            additionalFiles: [
                "Documentation/**/*.md",
        ...
        // Then
        XCTAssertEqual(model.name, "SomeWorkspace")
        XCTAssertEqual(model.additionalFiles.sorted(by: { $1.path > $0.path }), files.map { .file(path: $0) })
    }
    ...
    func test_loadWorkspace_withFolderReferences() throws {
        ...
        // Then
        XCTAssertEqual(model.name, "SomeWorkspace")
        XCTAssertEqual(model.additionalFiles.sorted(by: { $1.path > $0.path }), [
    ...
    func test_loadWorkspace_withInvalidProjectsPaths() throws {
        ...
        No projects found at: \(temporaryPath)/A
        No projects found at: \(temporaryPath)/B
        """)
        XCTAssertEqual(model.projects, [])
    }
