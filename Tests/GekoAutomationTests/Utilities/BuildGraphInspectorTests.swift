...
final class BuildGraphInspectorTests: GekoUnitTestCase {
    ...
    func test_workspacePath() throws {
        ...
        try FileHandler.shared.touch(workspacePath.appending(component: Constants.gekoGeneratedFileName))
    ...
    func test_workspacePath_when_no_geko_workspace_is_present() throws {
        // Given
        let path = try temporaryPath()
        let workspacePath = path.appending(component: "App.xcworkspace")
        try FileHandler.shared.createFolder(workspacePath)
    ...
    func test_workspacePath_when_multiple_workspaces_are_present() throws {
        // Given
        let path = try temporaryPath()
        let nonGekoWorkspacePath = path.appending(components: "SPM.xcworkspace")
        try FileHandler.shared.createFolder(nonGekoWorkspacePath)
        let workspacePath = path.appending(component: "GekoApp.xcworkspace")
        try FileHandler.shared.createFolder(workspacePath)
        try FileHandler.shared.touch(workspacePath.appending(component: Constants.gekoGeneratedFileName))
