...
class AbsolutePathTests: XCTestCase {
    ...
    func testStringInitialization() throws {
        ...
        let abs4 = try AbsolutePath(validating: "/a/b/c", relativeTo: base)
        XCTAssertEqual(abs4, try AbsolutePath(validating: "/a/b/c"))
        let abs5 = try AbsolutePath(validating: "./a/b/c", relativeTo: base)
        XCTAssertEqual(abs5, try AbsolutePath(validating: "/base/path/a/b/c"))
        let abs6 = try AbsolutePath(validating: "~/bla", relativeTo: base)  // `~` isn't special
        XCTAssertEqual(abs6, try AbsolutePath(validating: "/base/path/~/bla"))
    }
    ...
    func testCombinationsAndEdgeCases() {
        ...
        XCTAssertEqual(RelativePath("../a").pathString, "../a")
        XCTAssertEqual(RelativePath("../.a").pathString, "../.a")
        XCTAssertEqual(RelativePath("../.тест").pathString, "../.тест")
    ...
    func testRelativePathFromAbsolutePaths() {
        XCTAssertEqual(
            AbsolutePath("/").relative(to: AbsolutePath("/")),
            RelativePath(".")
        )
        XCTAssertEqual(
            AbsolutePath("/a/b/c/d").relative(to: AbsolutePath("/")),
            RelativePath("a/b/c/d")
        )
        XCTAssertEqual(
            AbsolutePath("/").relative(to: AbsolutePath("/a/b/c")),
            RelativePath("../../.."))
        XCTAssertEqual(
            AbsolutePath("/a/b/c/d").relative(to: AbsolutePath("/a/b")),
            RelativePath("c/d")
        )
        XCTAssertEqual(
            AbsolutePath("/a/b/c/d").relative(to: AbsolutePath("/a/b/c")),
            RelativePath("d")
        )
        XCTAssertEqual(
            AbsolutePath("/a/b/c/d").relative(to: AbsolutePath("/a/c/d")),
            RelativePath("../../b/c/d")
        )
        XCTAssertEqual(
            AbsolutePath("/a/b/c/d").relative(to: AbsolutePath("/b/c/d")),
            RelativePath("../../../a/b/c/d")
        )
    }
    ...
    func testAbsolutePathValidation() {
        XCTAssertNoThrow(try AbsolutePath(validatingAbsolutePath: "/a/b/c/d"))

        XCTAssertThrowsError(try AbsolutePath(validatingAbsolutePath: "~/a/b/d")) { error in
    ...
    func testRelativePathValidation() {
        ...
        XCTAssertThrowsError(try RelativePath(validatingRelativePath: "/a/b/d")) { error in
    ...
    func testCodable() throws {
        ...
        do {
            ...
            XCTAssertNoThrow(try JSONDecoder().decode(Foo.self, from: data))
        ...
        do {
            let data = try JSONEncoder().encode(Baz(path: "/foo"))
            XCTAssertNoThrow(try JSONDecoder().decode(Bar.self, from: data))
        }
    }
