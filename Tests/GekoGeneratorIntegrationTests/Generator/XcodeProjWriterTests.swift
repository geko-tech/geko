...
final class XcodeProjWriterTests: GekoTestCase {
    ...
    func test_generate_replacesProjectSharedSchemes() throws {
        ...
        // Then
        let fileHandler = FileHandler.shared
        let schemes = fileHandler.glob(xcodeProjPath, glob: "**/*.xcscheme").map(\.basename).sorted()
    ...
    func test_generate_replacesWorkspaceSharedSchemes() throws {
        ...
        // Then
        let fileHandler = FileHandler.shared
        let schemes = fileHandler.glob(xcworkspacePath, glob: "**/*.xcscheme").map(\.basename).sorted()
        XCTAssertEqual(schemes, [
            "SchemeA.xcscheme",
            "SchemeC.xcscheme",
        ])
