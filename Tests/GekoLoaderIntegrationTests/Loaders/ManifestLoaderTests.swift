...
final class ManifestLoaderTests: GekoTestCase {
    var subject: CompiledManifestLoader!

    override func setUp() {
        super.setUp()
        subject = CompiledManifestLoader()
    }
    ...
    func test_loadPackageSettings() throws {
        ...
        try FileHandler.shared.createFolder(temporaryPath.appending(component: Constants.gekoDirectoryName))
        try content.write(
            to: manifestPath.url,
            atomically: true,
            encoding: .utf8
    ...
    func test_loadWorkspace() throws {
        ...
        let content = """
        import ProjectDescription
        let workspace = Workspace(name: "geko", projects: [])
        """
