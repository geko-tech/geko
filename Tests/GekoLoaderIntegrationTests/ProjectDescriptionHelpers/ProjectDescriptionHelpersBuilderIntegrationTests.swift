...
final class ProjectDescriptionHelpersBuilderIntegrationTests: GekoTestCase {
    ...
    func test_build_when_the_helpers_is_a_dylib() throws {
        ...
        let helpersPath = path
            .appending(try RelativePath(validating: "\(Constants.gekoDirectoryName)/\(Constants.helpersDirectoryName)"))
        try FileHandler.shared.createFolder(path.appending(component: Constants.gekoDirectoryName))
