...
final class ProjectGroupsTests: XCTestCase {
    ...
    func test_projectGroup_with_reference() throws {
        // Given
        let target1 = Target.test(filesGroup: .group(name: "A"))
        let target2 = Target.test(filesGroup: .groupReference(name: "B", path: "/path/to/sourcesB"))
        let project = Project.test(
            path: .root,
            sourceRootPath: .root,
        ...
        XCTAssertEqual((try? subject.projectGroup(named: "B"))?.path, "/path/to/sourcesB")
    }
