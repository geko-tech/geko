...
final class ProjectFileElementsTests: GekoUnitTestCase {
    ...
    override func setUp() {
        ...
        subject = ProjectFileElements(enforceExplicitDependencies: false)
    }
    ...
    func test_projectFiles() {
        ...
        // When
        let files = subject.projectFiles(project: project, buildableFolders: [])
    ...
    func test_targetFiles() throws {
        ...
        let target = Target.test(
            ...
            settings: settings,
            sources: [SourceFiles(paths: [
                try AbsolutePath(validating: "/project/file.swift")
            ])],
            resources: [
            ...
            copyFiles: [
                CopyFilesAction(
                    ...
                    subpath: "Templates",
                    files: [
                        .file(path: "/project/geko.rtfd"),
                        .file(path: "/project/geko.rtfd/TXT.rtf"),
                    ]
                )
            ],
            coreDataModels: [
                CoreDataModel(
                    try AbsolutePath(validating: "/project/model.xcdatamodeld"),
        ...
        // When
        let files = try subject.targetFiles(target: target, buildableFolders: [])
        ...
        XCTAssertTrue(
            files.isSuperset(of: [
                ...
                GroupFileElement(path: "/project/geko.rtfd", group: target.filesGroup),
                GroupFileElement(path: "/project/geko.rtfd/TXT.rtf", group: target.filesGroup),
    ...
    func test_generateProducts_stableOrder() throws {
        for _ in 0..<5 {
            let pbxproj = PBXProj()
            let subject = ProjectFileElements(enforceExplicitDependencies: false)
            let targets: [Target] = [
    ...
    func test_generatePath_whenGroupIsSpecified_with_reference() throws {
        // Given
        let pbxproj = PBXProj()
        let path = try AbsolutePath(validating: "/a/b/c/file.swift")
        let fileElement = GroupFileElement(path: path, group: .groupReference(name: "SomeGroup", path: "/a/b"))
        let project = Project.test(
            path: .root,
            sourceRootPath: .root,
            xcodeProjPath: AbsolutePath.root.appending(component: "Project.xcodeproj"),
            filesGroup: .groupReference(name: "SomeGroup", path: "/a/b")
        )
        let sourceRootPath = try AbsolutePath(validating: "/a/project/")
        let groups = ProjectGroups.generate(project: project, pbxproj: pbxproj)
        ...
        // Then
        let group = try XCTUnwrap(groups.sortedMain.group(named: "SomeGroup"))
        XCTAssertEqual(group.name, "SomeGroup")
        XCTAssertEqual(group.path, "/a/b")
        XCTAssertEqual(group.sourceTree, .group)

        let cGroup: PBXGroup = group.children.first! as! PBXGroup
        XCTAssertNil(cGroup.name)
        XCTAssertEqual(cGroup.path, "c")
        XCTAssertEqual(cGroup.sourceTree, .group)
