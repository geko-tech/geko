...
final class TargetLinterTests: GekoUnitTestCase {
    ...
    func test_lint_when_target_has_invalid_bundle_identifier() {
        let XCTAssertInvalidBundleId: (String) -> Void = { bundleId in
            ...
            let got = self.subject.lint(target: target)
            let reason =
                "Invalid bundle identifier '\(bundleId)' for target 'Target'. This string must be a uniform type identifier (UTI) that contains only alphanumeric (A-Z,a-z,0-9), hyphen (-), and period (.) characters."
    ...
    func test_lint_when_ios_bundle_has_sources() {
        ...
        let bundle = Target.empty(
            ...
            product: .bundle,
            sources: [
                SourceFiles(paths: ["/path/to/some/source.swift"]),
            ],
            resources: []
        )
    ...
    func test_lint_when_deployment_target_version_is_valid() {
        ...
        for version in validVersions {
            ...
            XCTDoesNotContainLintingIssue(
                got,
                LintingIssue(reason: "The version \"\(version)\" of deployment target for \(target.name) is incorrect", severity: .error)
            )
        }
    }

    func test_lint_when_deployment_target_version_is_invalid() {
        let validVersions = ["geko", "geko9.0.1", "1.0geko", "10_0", "1_1_3"]
        for version in validVersions {
            // Given
            let target = Target.test(platform: .macOS, deploymentTarget: .macOS(version))
            ...
            // Then
            XCTContainsLintingIssue(got, LintingIssue(reason: "The version \"\(version)\" of deployment target for \(target.name) is incorrect", severity: .error))
        }
    }
    ...
    func test_lint_when_target_has_duplicate_dependencies_specified() {
        let testDependency: TargetDependency = .sdk(name: "libc++.tbd", type: .library, status: .optional)
    ...
    func test_lint_when_target_has_non_existing_core_data_models() throws {
        ...
        let target = Target.test(coreDataModels: [
            CoreDataModel(dataModelPath, versions: [], currentVersion: "1.0.0"),
        ])
    ...
    func test_lint_when_target_has_valid_codegen_sources() throws {
        ...
        let target = Target.empty(
            name: "MyTarget",
            sources: [
                SourceFiles(paths: ["/project/Source.swift"]),
                SourceFiles(paths: ["/project/Invalid.swift"], codeGen: .project),
                SourceFiles(paths: ["/project/Unspecified.intentdefinition"]),
                SourceFiles(paths: ["/project/Valid.intentdefinition"], codeGen: .private),
                SourceFiles(paths: ["/project/Valid.mlmodel"], codeGen: .disabled),
            ]
        )
        ...
        // Then
        XCTContainsLintingIssue(got, .init(
            reason: "Target '\(target.name)' has a source file at path \(target.sources[1].paths[0]) with unsupported `codeGen` attributes. Only intentdefinition and mlmodel are known to support this.",
            severity: .warning
        ))
    }
}
