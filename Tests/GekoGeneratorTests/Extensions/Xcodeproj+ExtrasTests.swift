...
class XcodeprojExtrasTests: XCTestCase {
    ...
    func test_setting_target_name_when_target_exists() throws {
        // Given
        let configuration = XCBuildConfiguration(name: "test")
        let target = PBXTarget(name: "some_name")

        // When
        let value = try configuration.resolveBuildSetting(
            key: "TARGET_NAME",
            projectDirPath: .root,
            project: PBXProj(),
            rootTarget: target
        ...
        // Then
        XCTAssertEqual(try value?.stringValue(), "some_name")
    }

    func test_setting_target_name_when_target_nil() throws {
        // Given
        let configuration = XCBuildConfiguration(name: "test")
